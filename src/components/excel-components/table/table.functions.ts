...
export function resizer($source: DOM, event: any): Promise<any> {
  return new Promise((resolve) => {
    ...
    document.onmousemove = (e: MouseEvent) => {
      if (resizeType === "column") {
        ...
        newWidth = Math.round(parentCoord.width + delta);
        $resizer.css({ right: -delta + "px" });
      } else if (resizeType === "row") {
        $resizer.css({ opacity: 1, right: "-5000px" });
        const delta = parentCoord.bottom - e.clientY;
        newHeight = Math.round(parentCoord.height - delta);
        $resizer.css({ bottom: delta + "px" });
      }
    };
    document.onmouseup = () => {
      if (resizeType === "column") {
        const columnName = $parent.data.columnName;
        $parent.css({ width: `${newWidth}px` });
        $source.getAllBySelector(`[data-cell-column="${columnName}"]`)
            .forEach((el: any) => {
              el.style.width = newWidth + "px";
            });
        if (newWidth) {
          resolve({ columnName: columnName, columnWidth: newWidth, type: resizeType });
        }
      } else if (resizeType === "row") {
        const rowName = $parent.data.rowName;
        $parent.css({ height: `${newHeight}px` });
        if (newHeight) {
          resolve({ rowName: rowName, rowHeight: newHeight, type: resizeType });
        }
      }
